...
client.on(Events.ClientReady, async (c) => {
  ...
  player.on("error", (error) => {
    console.error(
      `[Audio player] Error: ${error.message} with resource ${error.resource}`,
    );
  ...
  connection.on("debug", (d) => {
    console.log(`[Connection] DEBUG: ${d}`);
  });
  ...
  connection.on(VoiceConnectionStatus.Ready, async (_oldState, _newState) => {
    ...
    if (!urls) throw new Error("No URLs found in environment variable");
